...
export default function Perfil() {
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      // 2. Upload de avatar se seleccionado
      if (avatarFile) {
      ...
      setMsg(lang==='en'?'Profile updated!':'Perfil actualizado com sucesso!');
      setEditing(false);
      setAvatarFile(null);
      setPreview(null);
    } catch (err) {
          ...
          <div className="card" style={{ padding:'var(--s8)' }}>
            ...
            <div style={{ textAlign:'center', marginBottom:'var(--s6)' }}>
              <div style={{ position:'relative', display:'inline-block' }}>
                ...
                >
                  <IconCamera className="icon icon--sm" style={{ color:'white' }} />
                </button>
              </div>
              ...
              <div style={{ marginTop:'var(--s3)' }}>
                <div style={{ fontFamily:'var(--display)', fontSize:'var(--t-xl)', fontWeight:'var(--w-black)', color:'var(--ink-900)' }}>
                  {user?.name}
                </div>
                <div style={{ display:'flex', justifyContent:'center', gap:'var(--s2)', marginTop:'var(--s2)' }}>
                  <Badge level={user?.level} />
                  {user?.badge && <span className={`seal seal--${user.badge}`}>{user.badge_label}</span>}
                  {user?.verified && (
                    <span style={{ fontSize:12, color:'#1E5631', fontWeight:700, background:'#E6F4EA', padding:'2px 8px', borderRadius:999 }}>
            ...
            {msg   && <div style={{ background:'#E6F4EA', color:'#1E5631', padding:'10px 14px', borderRadius:8, fontSize:13, fontWeight:600, marginBottom:16 }}>{msg}</div>}
            {error && <div style={{ background:'var(--red-soft)', color:'var(--red)', padding:'10px 14px', borderRadius:8, fontSize:13, fontWeight:600, marginBottom:16 }}>{error}</div>}

            {editing ? (
              <form onSubmit={handleSave} style={{ display:'flex', flexDirection:'column', gap:'var(--s4)' }}>
                {[
                  ...
                  { key:'identifier', label: lang==='en'?'Identifier (optional)':'Identificador (opcional)', type:'text', placeholder: lang==='en'?'Ex: React Developer, Full Stack...':'Ex: Programador React, Full Stack...' },
                ].map(field => (
                  <div key={field.key} className="field">
                    ...
                    <input
                      className="input"
                      type={field.type}
                      value={(form as any)[field.key] ?? form[field.key]}
                ...
                <div style={{ display:'flex', gap:'var(--s3)', justifyContent:'flex-end' }}>
                  <Button type="button" variant="secondary" style={{ width:'auto' }}
                    onClick={() => { setEditing(false); setForm({ name: user?.name||'', email: user?.email||'', identifier: user?.identifier||'' }); setPreview(null); setAvatarFile(null); setError(''); }}>
                    {lang==='en'?'Cancel':'Cancelar'}
                  </Button>
                  <Button type="submit" loading={saving} style={{ width:'auto' }}>
                    <IconCheck className="icon icon--sm" /> {lang==='en'?'Save':'Guardar'}
                  </Button>
                </div>
              </form>
            ) : (
              <>
                <div style={{ display:'flex', flexDirection:'column', gap:0 }}>
                  {[
                    ...
                    { label: lang==='en'?'Identity':'Identidade',    value: user?.verified ? '✓ Verificada' : (lang==='en'?'Not verified':'Não verificada') },
                  ].map(row => (
                    <div key={row.label} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'var(--s3) 0', borderBottom:'var(--line)' }}>
                      <span style={{ fontSize:'var(--t-sm)', color:'var(--ink-400)', fontWeight:'var(--w-bold)' }}>{row.label}</span>
                      <span style={{ fontSize:'var(--t-sm)', color:'var(--ink-900)', fontWeight:'var(--w-medium)', textAlign:'right', maxWidth:'60%' }}>{row.value}</span>
                    </div>
                  ))}
                </div>
                <div style={{ display:'flex', gap:'var(--s3)', marginTop:'var(--s6)' }}>
                  <Button style={{ width:'auto' }} onClick={() => setEditing(true)}>
                    <IconEdit className="icon icon--sm" /> {lang==='en'?'Edit profile':'Editar perfil'}
                  </Button>
                  {!user?.verified && (
                    <Button variant="secondary" style={{ width:'auto' }} as="a" href="/verificacao">
