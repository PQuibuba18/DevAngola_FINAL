...
const verificationController = {
  ...
  async submit(req, res) {
    try {
      ...
      // Admin é verificado automaticamente — não precisa de revisão
      const userCheck = await db.query("SELECT role FROM users WHERE id=$1", [
        userId,
      ]);
      if (userCheck.rows[0]?.role === "admin") {
        await db.query(
          "UPDATE users SET verified=TRUE, verified_at=NOW(), verification_method='admin_auto' WHERE id=$1",
          [userId],
        );
        return res.json({
          status: "approved",
      ...
      if (existing.rows.length > 0) {
        ...
        if (status === "approved")
          return res
            .status(409)
            .json({ error: "A tua identidade já foi verificada." });
        if (status === "pending")
          return res
            .status(409)
            .json({ error: "Já tens um pedido em análise. Aguarda." });
      }
      ...
      if (!useCloudinary && localPath && fs.existsSync(localPath)) {
        ...
        ocrResult = {
          ...
          reason:
            "Análise automática não disponível com Cloudinary. Revisão manual.",
        };
      }
      ...
      if (status === "approved") {
        return res.json({
          ...
          verified: true,
          message:
            "Identidade verificada! O teu perfil recebeu o selo ✓ Angolano Verificado.",
        ...
        return res.status(400).json({
          status: "rejected",
          message:
            "Não foi possível confirmar a tua nacionalidade angolana. Certifica-te de enviar o BI ou Passaporte angolano e tenta novamente.",
        });
      } else {
        return res.json({
          status: "pending",
          message:
            "Documento recebido. Será analisado manualmente pelo administrador em 1-3 dias úteis.",
        });
      }
    } catch (err) {
      ...
      return res
        .status(500)
        .json({ error: "Erro interno ao processar documento." });
    }
  },
  ...
  async adminApprove(req, res) {
    try {
      ...
      try {
        ...
        await client.query("COMMIT");
        return res.json({
          message: "Identidade aprovada. Documento eliminado.",
        });
      } catch (err) {
        await client.query("ROLLBACK");
        throw err;
